...
public class Driver {
        ...
        while (true) {
            ...
            switch (getInput(input)) {
                ...
                case "7":
                    ...
                    System.out.println(movies[0].getTicketsSold() + " tickets have been sold for the Dumbo Movie.");
    ...
    private static void option2(StringBuilder input, ParallelQueue lines, Theater[] movies) throws IOException {
        ...
        if (!lines.isEmpty()) {
            ...
            if (customer.getMovieName().equals("Dumbo"))
                assignMovie0(input, movies, customer, true);
            else
     ...
     *
     * @param line ParallelQueue that represents lines
     */
    ...
    /**
     * This method checks if the party can be seated
     * in the Dumbo movie (movie #1). If they cannot be
     * seated, then prompt to be seated in the other
     * movie.
     *
     * @param input
     * @param movies
     ...
     * @throws IOException
     */
    private static void assignMovie0(StringBuilder input, Theater[] movies, Customer customer, boolean firstTime) throws IOException {
        if (movies[0].assignSeats(customer)) {
            ...
            //check if the other movie has enough seats
            if (movies[1].hasEnoughSeats(customer.getSizeOfParty())) {
                if (firstTime) {    //if it is the first time checking the other movie
                    System.out.println("Sorry. This movie is sold out.");
                    System.out.print("Would you like to see the other movie(Y/N)?");
                    if (getInput(input).equalsIgnoreCase("Y"))  //if Y then attempt the other movie
                        assignMovie1(input, movies, customer, false);
                    else
                        System.out.println("Customer " + customer.getName() + " has left the Movie Theater.");
                } else {
                    //if it's the second time checking, then both movies are sold out
                    System.out.println("Sorry. Both movies are sold out. Good bye!");
                    System.out.println("Customer " + customer.getName() + " has left the Movie Theater.");
                }
            } else {
                //if there is not enough seats in the other movie
                System.out.println("Sorry. Both movies are sold out. Good bye!");
                System.out.println("Customer " + customer.getName() + " has left the Movie Theater.");
            }
        }
    ...
    /**
     * This method checks if the party can be seated
     * in the Shazam! movie (movie #2). If they cannot be
     * seated, then prompt to be seated in the other
     * movie.
     *
     * @param input
    ...
    private static void assignMovie1(StringBuilder input, Theater[] movies, Customer customer, boolean firstTime) throws IOException {
        if (movies[1].assignSeats(customer)) {
            ...
            if (movies[0].hasEnoughSeats(customer.getSizeOfParty())) {
                if (firstTime) {
                    System.out.println("Sorry. This movie is sold out.");
                    System.out.print("Would you like to see the other movie(Y/N)?");
                    if (getInput(input).equalsIgnoreCase("Y"))
                        assignMovie0(input, movies, customer, false);
                    else
                        System.out.println("Customer " + customer.getName() + " has left the Movie Theater.");
                } else {
                    System.out.println("Sorry. Both movies are sold out. Good bye!");
